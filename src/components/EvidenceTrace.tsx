    ...
    <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="cs-focusable bg-[#0e1118] border border-[#1c212c] font-mono text-[12px] text-white rounded-lg pl-8 pr-2.5 py-2 w-48 focus:border-[#7dd3fc]/50 placeholder:text-[#3d4350] transition-colors"
              />
            </div>
            <select
              value={filter} onChange={(e) => setFilter(e.target.value)}
              className="cs-focusable bg-[#0e1118] border border-[#1c212c] font-mono text-[12px] text-white rounded-lg px-2.5 py-2 focus:border-[#7dd3fc]/50 transition-colors"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_minmax(0,1fr)] gap-4 items-start">
        ...
        <div className="cs-panel p-3.5 space-y-2 max-h-[640px] overflow-y-auto cs-scroll">
          ...
          {filtered.length === 0 && (
            <div className="py-10 text-center font-mono text-[12px] leading-relaxed text-[#3d4350]">
          ...
          {filtered.map((c) => (
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-1.5">
                <span className="font-mono text-[10px] tracking-[0.08em] text-[#3d4350]">{c.type}</span>
                <StateTag state={c.verification_status} />
              </div>
              <p className="cs-body text-white line-clamp-2">{c.statement}</p>
              <div className="mt-1.5 font-mono text-[11px] text-[#5d6474]">{c.origin_agent} · {c.evidence_refs.length} refs</div>
            </button>
          ))}
        </div>

        {/* Backward chain */}
        <div className="cs-panel p-5 md:p-7">
          {!selected ? (
            <div className="py-20 text-center font-mono text-[12px] text-[#3d4350]">Select a claim to trace its provenance.</div>
          ) : (
            <div className="space-y-6" key={selected.claim_id}>
              <div className="border-b cs-hairline pb-5 cs-fade-in">
                <div className="font-mono text-[11px] text-[#3d4350] mb-1.5">FINAL CONCLUSION · <span className="text-[#7dd3fc]">{selected.claim_id}</span></div>
                <h3 className="cs-display-sm text-white">{selected.statement}</h3>
                <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1.5 font-mono text-[12px] text-[#5d6474]">
                  ...
                  <span>confidence <strong className="text-[#7dd3fc] tabular-nums">{(selected.confidence * 100).toFixed(0)}%</strong></span>
                  <StateTag state={selected.verification_status} />
                </div>
                {selected.verification_reason && (
                  <p className="mt-3 font-mono text-[12px] leading-relaxed text-[#8b93a5] rounded-lg border border-[#1c212c] bg-[#0e1118] p-3">{selected.verification_reason}</p>
                )}
                <GroundedNote className="mt-2.5" />
              </div>
              ...
              <div className="relative ml-2 border-l border-[#262c39] pl-5 space-y-4">
                {steps.map((s, i) => {
                  ...
                  return (
                    <div key={i} className="relative cs-rise" style={{ animationDelay: `${Math.min(i * 70, 420)}ms` }}>
                      <span className="absolute -left-[26px] top-1.5 w-2.5 h-2.5 rounded-full bg-[#060709] border-2" style={{ borderColor: tone }} />
                      <div className={`rounded-lg border p-3.5 transition-colors ${missing ? 'border-[#fda4af]/30 bg-[#fda4af]/5' : 'border-[#1c212c] bg-[#0e1118] hover:border-[#343b4c]'}`}>
                        <div className="flex items-center justify-between gap-2 font-mono text-[11px] mb-1.5">
                        ...
                        <p className="cs-body text-white">{s.description}</p>
                        {s.level === 'EVIDENCE' && (
                          <div className="mt-2.5 space-y-1.5">
                            ...
                            <pre className="font-mono text-[12px] leading-relaxed cs-inset p-3 text-[#7dd3fc] whitespace-pre-wrap overflow-x-auto cs-scroll">{String(s.details['extract'] ?? '')}</pre>
                          </div>
                        )}
                      </div>
                    </div>
